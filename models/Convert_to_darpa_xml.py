...
def print_entities(entities, curr_docum, curr_anot):
...
def run_program():
    ...
    if len(sys.argv) != 2:
        raise NotImplementedError(
            'Program only takes one argument: the 10-column format from Akash, with the tags in the end')
    with codecs.open(sys.argv[1], encoding='utf-8', mode='r') as input_file:
    ...
    for i, line in enumerate(lines):
        if len(line) > 2:
            ...
            if line_split[-1][0] == 'B':
                if in_entity:
                    print_entities(entities, curr_docum, curr_anot)
                    # restart
                    entities[0] = []
                    entities[1] = None
                    entities[2] = -1
