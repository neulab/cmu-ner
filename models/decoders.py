...
class chain_CRF_decoder(Decoder):
    ...
    def decode_loss(self, src_encodings, tgt_tags,use_partial, known_tags, tag_to_id, UNK):
	...
	if use_partial:
	    gold_score = self.score_one_sequence_partial(tag_scores, tgt_tags, batch_size,known_tags, tag_to_id, UNK)
	else:
    ...
    def makeMask(self, batch_size, known_tags, tag_to_id, tags, index, UNK):
	...
	for idx, w0_si in enumerate(known_tags[index]):
		...
		if tags[index][idx] == UNK:
		    possible_labels = ["B-LOC", "B-PER", "B-ORG", "B-MISC", "I-LOC", "I-PER", "I-ORG", "I-MISC", "O"]
    ...
    def score_one_sequence_partial(self, tag_scores, tags, batch_size, known_tags, tag_to_id, UNK):
	transpose_transition_score = dy.parameter(self.transition_matrix)
	alpha_tm1 = transpose_transition_score[self.start_id] + tag_scores[0]

	mask_w_0_all_s = self.makeMask(batch_size, known_tags, tag_to_id, tags, 0, UNK)
	i = 1
	...
	for tag_score in tag_scores[1:]:
	    ...
	    alpha_t = alpha_tm1 + transpose_transition_score + tag_score
	    alpha_tm1 = log_sum_exp_dim_0(alpha_t)  # (tag_size, batch_size)
	    mask_w_i_all_s = self.makeMask(batch_size, known_tags, tag_to_id,tags, i,UNK)
