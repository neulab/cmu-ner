...
def run_program_2(input, output, setEconll):
    if input is not None and setEconll is not None:
        with codecs.open(input, 'r') as input_file:
            lines = input_file.readlines()
        tags = []
        for i, line in enumerate(lines):
            if len(line) >= 2:
        ...
        with codecs.open(setEconll, 'r') as input_file:
            lines = input_file.readlines()
        assert len(output_lines) == len(lines)
        with codecs.open(output, 'w') as output_file:
            ctr = -1
            for line in lines:
                if len(line) > 2:
                    ctr += 1
